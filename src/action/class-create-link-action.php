 ...
 * @version
 */
class Create_Link_Action extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'PRETTY_LINKS_CREATE_LINK' );

		// Reuseable trigger meta, can be used as the ID of a field OR group of fields.
		$this->set_action_meta( 'PRETTY_LINKS_CREATE_LINK_META' );
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr__(
					'Create a pretty link with {{a specific target URL:%1$s}}',
					'automator-sample'
				),
		...
		// Sentence that appears in the trigger list drop down.
		$this->set_readable_sentence( esc_attr__( 'Create a pretty link with {{a specific target URL}}', 'automator-sample' ) );
